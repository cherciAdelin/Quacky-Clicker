...
func special_up(upgrade: String, cost_inc: int, val_inc: float, _active_up: Control, hat_texture: Texture2D):
	...
	var passed_value := float(Global.hats[upgrade]["value"])
		...
		#active_up.visible = true
		#DuckHatChange.emit(hat_texture)
	if(Global.hats[upgrade]["level"] == 1):
		DuckHatChange.emit(hat_texture)
	return passed_value
...
func UI_change_hats(upgrade: String, costLabel: Control, lvlLabel: Control):
	costLabel.text = str(Global.hats[upgrade]["cost"]) + "shell"
...
func _on_main_up_menu() -> void:
	...
	UI_change_hats("straw_hat", strawHatCost, strawHatLVL)
	UI_change_hats("cowboy_hat", cowboyHatCost, cowboyHatLVL)
	UI_change_hats("witch_hat", witchHatCost, witchHatLVL)
	UI_change_hats("wizard_hat", wizardHatCost, wizardHatLVL)
...
func _on_up_button_pressed_straw_hat() -> void:
	if(Global.eggshell_currency < Global.hats["straw_hat"]["cost"]):
		...
		var eggshell_mult = special_up("straw_hat", 50, 0.1, null, strawHatTexture)
...
func _on_up_button_pressed_cowboy_hat() -> void:
	if(Global.eggshell_currency < Global.hats["cowboy_hat"]["cost"]):
		...
		var eggshell_limitbreak = special_up("cowboy_hat", 150, 1, null, cowboyHatTexture)
...
func _on_up_button_pressed_witch_hat() -> void:
	if(Global.eggshell_currency < Global.hats["witch_hat"]["cost"]):
		...
		var active_pw_cdr = special_up("witch_hat", 300, 2.5, null, witchHatTexture)
		Global.active_powerup_cooldown -= active_pw_cdr
...
func _on_up_button_pressed_wizard_hat() -> void:
	if(Global.eggshell_currency < Global.hats["wizard_hat"]["cost"]):
		...
		var active_powerup_mult = special_up("wizard_hat", 500, 0.1, null, wizardHatTexture)
